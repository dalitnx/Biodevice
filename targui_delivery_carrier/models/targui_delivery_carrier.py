...
class TarguiData():
    ...
    APIEND = {
        "test": {
            'token': 'http://51.15.161.42:88/WebServiceExterne/get_token',
            'shipment': 'http://51.15.161.42:88/WebServiceExterne/pos_create',
            'label': 'http://51.15.161.42:88/WebServiceExterne/get_label',
        },
        "production": {
...
class TarguiDeliveryCarrier(models.Model):
    ...
    def targui_rate_shipment(self, order):
        ...
        config = self.wk_get_carrier_settings(
            ['prod_environment', 'targui_test_username', 'targui_test_password', 'targui_prod_username', 'targui_prod_password', 'targui_mr_code', 'targui_fixed_rate'])
        config['targui_enviroment'] = 'production' if config['prod_environment'] else 'test'
